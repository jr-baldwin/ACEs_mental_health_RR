Main analysis on complete case data - ALSPAC & ABCD
================
Jessie Baldwin

-   [Load packages](#load-packages)
-   [Load imputed data and create
    variables](#load-imputed-data-and-create-variables)
-   [Hypothesis 1A](#hypothesis-1a)
    -   [Association between PGSs for psychopathology and
        ...
        -   [Define function to run regression
            models](#define-function-to-run-regression-models)
        ...
        -   [ALSPAC - Get correlation between ACEs for aggregate
            model](#alspac---get-correlation-between-aces-for-aggregate-model)
        -   [ALSPAC - Prepare to run aggregate
            model](#alspac---prepare-to-run-aggregate-model)
        -   [ALSPAC - Run aggregate
            meta-analysis](#alspac---run-aggregate-meta-analysis)
    ...
    -   [ALSPAC - Association between negative control PGSs and
        ...
        -   [Define function to run regression models for negative
            controls](#define-function-to-run-regression-models-for-negative-controls)
    ...
    -   [ALSPAC - label dataframes to specify ALSPAC ahead of
        plotting](#alspac---label-dataframes-to-specify-alspac-ahead-of-plotting)
        ...
        -   [ABCD - Run aggregate
            meta-analysis](#abcd---run-aggregate-meta-analysis)
        -   [ABCD - Prepare to make forest
            plot](#abcd---prepare-to-make-forest-plot)
        -   [ABCD - Run regressions for negative
            controls](#abcd---run-regressions-for-negative-controls)
        -   [ABCD - Prepare to run aggregate model for negative
            controls](#abcd---prepare-to-run-aggregate-model-for-negative-controls)
        -   [ABCD - Run aggregate model for negative
            controls](#abcd---run-aggregate-model-for-negative-controls)
        -   [ABCD - Prepare to make forest plot for negative
            controls](#abcd---prepare-to-make-forest-plot-for-negative-controls)
    -   [ABCD - save dataframes ahead of
        plotting](#abcd---save-dataframes-ahead-of-plotting)
...
-   [Hypothesis 1B](#hypothesis-1b)
    ...
    -   [ABCD - Run pairwise comparisons to test which polygenic scores
        differ](#abcd---run-pairwise-comparisons-to-test-which-polygenic-scores-differ)
    -   [ALSPAC and ABCD - Plot results for
        Hyp1b](#alspac-and-abcd---plot-results-for-hyp1b)
-   [Hypothesis 1C](#hypothesis-1c)
    ...
    -   [Combine ALSPAC and ABCD figures for differences between
        ACEs](#combine-alspac-and-abcd-figures-for-differences-between-aces)
...
-   [ALSPAC - Specify SEM](#alspac---specify-sem-2)
    ...
    -   [ALSPAC - Define function to extract
        results](#alspac---define-function-to-extract-results)
    ...
    -   [ALSPAC - Run models for externalising
        problems](#alspac---run-models-for-externalising-problems)
    ...
    -   [ALSPAC - Convert cohen’s d values to r for
        table](#alspac---convert-cohens-d-values-to-r-for-table)
-   [ABCD - Specify SEM](#abcd---specify-sem-2)
    ...
    -   [ALSPAC - Extract results for externalising problems into a
        dataframe](#alspac---extract-results-for-externalising-problems-into-a-dataframe-1)
    -   [ABCD - Aggregate results for proportions explained by polygenic
        scores](#abcd---aggregate-results-for-proportions-explained-by-polygenic-scores-1)
    -   [ABCD - Convert cohen’s d values to r for
        table](#abcd---convert-cohens-d-values-to-r-for-table)
    -   [ALSPAC and ABCD - make table with results for observed
        polygenic scores (Hypothesis
        2A)](#alspac-and-abcd---make-table-with-results-for-observed-polygenic-scores-hypothesis-2a)
        -   [ALSPAC - Repeat analysis for negative control polygenic
            scores](#alspac---repeat-analysis-for-negative-control-polygenic-scores)
        -   [ALSPAC - Run models for negative controls and internalising
            problems](#alspac---run-models-for-negative-controls-and-internalising-problems)
...
-   [Hypothesis 2B](#hypothesis-2b)
    ...
    -   [ALSPAC - Step 1. Obtain correlation between the ACE and the
        mental health
        outcome](#alspac---step-1-obtain-correlation-between-the-ace-and-the-mental-health-outcome)
    ...
    -   [ALSPAC - Run Gsens for externalising
        problems](#alspac---run-gsens-for-externalising-problems-1)
    -   [ABCD - Pool gsens results using all PGSs and format for
        table/plot](#abcd---pool-gsens-results-using-all-pgss-and-format-for-tableplot)
    -   [Make table with ALSPAC and ABCD results for all PGSs for
        hypothesis
        2b](#make-table-with-alspac-and-abcd-results-for-all-pgss-for-hypothesis-2b)
...
# Maltreatment
n1 <- round(as.matrix(table(alspac_CC$maltreatment_0_9.5yrs)[2],0))
p1 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$maltreatment_0_9.5yrs))[2]),4)*100)
# Domestic violence
n2 <- round(as.matrix(table(alspac_CC$violence_between_parents_0_9.5yrs)[2],0))
p2 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$violence_between_parents_0_9.5yrs))[2]),4)*100)
# Parental mental illness
n3 <- round(as.matrix(table(alspac_CC$mental_health_problems_or_suicide_0_9.5yrs)[2],0))
p3 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$mental_health_problems_or_suicide_0_9.5yrs))[2]),4)*100)
# Parental substance abuse
n4 <- round(as.matrix(table(alspac_CC$substance_household_0_9.5yrs)[2],0))
p4 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$substance_household_0_9.5yrs))[2]),4)*100)
# Parental criminality
n5 <- round(as.matrix(table(alspac_CC$parent_convicted_offence_0_9.5yrs)[2],0))
p5 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$parent_convicted_offence_0_9.5yrs))[2]),4)*100)
# Parental separation
n6 <- round(as.matrix(table(alspac_CC$parental_separation_0_9.5yrs)[2],0))
p6 <- as.numeric(round(as.matrix(prop.table(table(alspac_CC$parental_separation_0_9.5yrs))[2]),4)*100)

prev_aces_alspac_c <- data.frame(ACE=c("Maltreatment", "Domestic violence", "Parental mental illness",
                              "Parental substance abuse", "Parental criminality", "Parental separation"),
                               N = c(n1, n2, n3, n4, n5, n6),
                               Perc = c(p1, p2, p3, p4, p5, p6))


### ABCD

# Maltreatment
n1 <- round(as.matrix(table(abcd_CC$maltreatment)/50)[2],0)
p1 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$maltreatment))[2]),4)*100)
# Domestic violence
n2 <- round(as.matrix(table(abcd_CC$domestic_violence)/50)[2],0)
p2 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$domestic_violence))[2]),4)*100)
# Parental mental illness
n3 <- round(as.matrix(table(abcd_CC$parental_psychopathology)/50)[2],0)
p3 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$parental_psychopathology))[2]),4)*100)
# Parental substance abuse
n4 <- round(as.matrix(table(abcd_CC$parental_substance)/50)[2],0)
p4 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$parental_substance))[2]),4)*100)
# Parental criminality
n5 <- round(as.matrix(table(abcd_CC$parental_criminality)/50)[2],0)
p5 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$parental_criminality))[2]),4)*100)
# Parental separation
n6 <- round(as.matrix(table(abcd_CC$parental_separation)/50)[2],0)
p6 <- as.numeric(round(as.matrix(prop.table(table(abcd_CC$parental_separation))[2]),4)*100)

prev_aces_abcd_c <- data.frame(ACE=c("Maltreatment", "Domestic violence", "Parental mental illness",
                              "Parental substance abuse", "Parental criminality", "Parental separation"),
                               N = c(n1, n2, n3, n4, n5, n6),
                               Perc = c(p1, p2, p3, p4, p5, p6))

# combine dataframes
prev_aces_i <- rbind(prev_aces_alspac_i, prev_aces_abcd_i)
...
# round p-values to 4DP for those greater than 0.0000 and edit other p-value annotation
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr>0.0001] <- format(round(df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr>0.0001], 4), nsmall=4)
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="4.34227255746934e-05"] <- "4.34e-05"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="9.48771263746785e-11"] <- "9.49e-11"
...
# round p-values to 4DP for those greater than 0.0000 and edit other p-value annotation
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr>0.0001] <- format(round(df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr>0.0001], 4), nsmall=4)
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="7.70165799081807e-05"] <- "7.70e-05"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="3.74019469557624e-08"] <- "3.74e-08"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="1.07702721837569e-07"] <- "1.08e-07"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="4.04224872010759e-05"] <- "4.04e-05"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="7.16345611793691e-05"] <- "7.16e-05"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="8.88951827909361e-06"] <- "8.89e-06"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="1.07702721837569e-07"] <- "1.08e-07"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="1.07702721837569e-07"] <- "1.08e-07"
df_forest_ace$long_results.p_fdr[df_forest_ace$long_results.p_fdr=="3.58616062368893e-05"] <- "3.59e-05"
```
