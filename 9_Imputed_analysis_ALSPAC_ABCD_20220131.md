...
all_imputations_alspac <- as.data.frame(bind_rows(unclass(a.out_alspac$imputations), .id = "m") %>%
  group_by(m))

# Maltreatment
p1 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$maltreatment_0_9.5yrs))[2]),4)*100)
# Domestic violence
p2 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$violence_between_parents_0_9.5yrs))[2]),4)*100)
# Parental mental illness
p3 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$mental_health_problems_or_suicide_0_9.5yrs))[2]),4)*100)
# Parental substance abuse
p4 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$substance_household_0_9.5yrs))[2]),4)*100)
# Parental criminality
p5 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$parent_convicted_offence_0_9.5yrs))[2]),4)*100)
# Parental separation
p6 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_alspac$parental_separation_0_9.5yrs))[2]),4)*100)

prev_aces_alspac_i <- data.frame(ACE=c("Maltreatment", "Domestic violence", "Parental mental illness",
...
# Maltreatment
p1 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$maltreatment_r))[2]),4)*100)
# Domestic violence
p2 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$domestic_violence))[2]),4)*100)
# Parental mental illness
p3 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$parental_psychopathology))[2]),4)*100)
# Parental substance abuse
p4 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$parental_substance))[2]),4)*100)
# Parental criminality
p5 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$parental_criminality))[2]),4)*100)
# Parental separation
p6 <- as.numeric(round(as.matrix(prop.table(table(all_imputations_abcd$parental_separation))[2]),4)*100)

prev_aces_abcd_i <- data.frame(ACE=c("Maltreatment", "Domestic violence", "Parental mental illness",
                              "Parental substance abuse", "Parental criminality", "Parental separation"),
                               Perc = c(p1, p2, p3, p4, p5, p6))
...
kable(prev_aces_i) %>% 
  ...
  pack_rows("ALSPAC", 1, 6, label_row_css = "color:black") %>%
  pack_rows("ABCD", 7, 12, label_row_css = "color:black")

# State we do not present descriptives for int / ext and PGSs as these variables were standardised to mean=0, SD=1
```
...
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr>0.00001] <- format(round(df_forest$long_results.p_fdr[df_forest$long_results.p_fdr>0.00001], 4), nsmall=4)
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="1.35571553983027e-11"] <- "1.36e-11"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="3.95759638927018e-06"] <- "3.96e-06"
...
# round p-values to 4DP for those greater than 0.0001 and edit other p-value annotation
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr>0.0001] <- format(round(df_forest$long_results.p_fdr[df_forest$long_results.p_fdr>0.0001], 4), nsmall=4)
# Manually edit those very small p-values to avoid them saying 0.0000
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="4.79747441772815e-09"] <- "4.80e-09"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="1.12966340282128e-07"] <- "1.13e-07"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="2.90391973312154e-05"] <- "2.90e-05"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="6.08309914960081e-05"] <- "6.08e-05"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="5.10857013605711e-05"] <- "5.11e-05"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="2.49140335029097e-08"] <- "2.49e-08"
df_forest$long_results.p_fdr[df_forest$long_results.p_fdr=="1.53164430471975e-05"] <- "1.53e-05"
```
